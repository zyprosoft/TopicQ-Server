...
class UserService extends \App\Service\BaseService
{
    ...
    public function statistic()
    {
        ...
        //获取非化身总数
        $totalRealUser = User::query()->selectRaw('select count(`user_id`) as total from user where `user_id` not in (select `avatar_user_id` from manager_avatar_user)')
            ->first()->getAttribute('total');
        //今日新增用户
        $totalRealUserToday = User::query()->selectRaw("select count(`user_id`) as total from user where `created_at` like '%$today%' and `user_id` not in (select `avatar_user_id` from manager_avatar_user)")
            ->first()->getAttribute('total');
